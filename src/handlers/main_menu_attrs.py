...
main_menu_attrs_router = Router()

# @main_menu_attrs_router.message_handler(Filters.text("Запостить") & Filters.state(States.custom_post_menu))


@main_menu_attrs_router.message_handler(regexp="^Листать посты$", state="*")
...
@main_menu_attrs_router.message_handler(regexp="^Отложка$", state="*")
...
@main_menu_attrs_router.message_handler(regexp="^Парсер$", state="*")
...
@main_menu_attrs_router.message_handler(regexp="^Вызвать парсер$", state=States.parser, run_task=True)
